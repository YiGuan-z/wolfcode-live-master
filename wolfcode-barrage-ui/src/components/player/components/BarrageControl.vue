  ...
  methods: {
    ...
    ClickSlider(e) {
      ...
      if (id === "slider" || id === "loaded" || id === "played") {
        ...
        this.$parent.Clearbarrage();
        this.$parent.$refs.webSocket.clientBarrageMsgRollingSend();
      }
    },
    //设置倍速
    SetSpeed(speed) {
    ...
    FullScreen() {
      if (!this.isFull) {
        let doc = this.$parent.$refs.videoBox;
